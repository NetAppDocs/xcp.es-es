...
[role="lead"]
El NFS `help` command muestra una lista de comandos, parámetros de comandos y una breve descripción de cada uno. La `help` Command es útil para principiantes que son nuevos en la herramienta XCP.
...
Utilice la `info` con el `help` comando para mostrar documentación, ejemplos y recomendaciones de ajuste.
