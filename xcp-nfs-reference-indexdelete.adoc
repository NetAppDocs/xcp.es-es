...
[role="lead"]
El NFS `indexdelete` el comando suprime los índices de catálogo.
...
----
====
En la siguiente tabla, se muestra el `indexdelete` parámetros y su descripción.
...
| <<nfs_indexdelete_match,indexdelete -match   >> | Sólo procesa los archivos y directorios que coinciden con el filtro. 


| <<nfs_indexdelete_loglevel,indexdelete -loglevel   >> | Establece el nivel de log; los niveles disponibles son INFO, DEBUG (por defecto: INFO). 
|===


== indexdelete -match <filter>

Utilice la `-match <filter>` con el `indexdelete` comando para procesar sólo los archivos y directorios que coinciden con el filtro.
...
Utilice la `-loglevel <name>` con el `indexdelete` Comando para definir el nivel de log; los niveles disponibles son INFO y DEBUG. El nivel predeterminado es INFO.
